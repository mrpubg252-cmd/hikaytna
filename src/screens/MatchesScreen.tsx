  ...
  return (
    <div className="min-h-screen bg-[#070708] text-white pb-32 selection:bg-red-600 selection:text-white font-sans">
      ...
      <main className="max-w-6xl mx-auto px-4 md:px-6 pt-24 space-y-6">
        ...
        {/* PREMIUM ROYAL HEADER & LOCAL CLOCK BAR */}
        <div className="flex flex-col md:flex-row items-center justify-between gap-4 bg-gradient-to-r from-red-950/20 via-zinc-900/40 to-black/60 border border-red-950/20 p-5 rounded-3xl shrink-0 shadow-xl">
          <div className="flex items-center gap-3 text-right">
            <div className="w-10 h-10 rounded-2xl bg-red-600/10 border border-red-500/20 flex items-center justify-center text-red-500 shadow-lg shrink-0">
              <Trophy className="w-5 h-5 animate-pulse" />
            </div>
            <div>
              <h1 className="text-lg md:text-xl font-black text-white">جدول المباريات والبث الحي التفاعلي</h1>
              <p className="text-[10px] text-zinc-400">بث فوري مستمر مع شات حماسي موحد ومباشر</p>
            </div>
          </div>

          <div className="flex items-center gap-2 bg-zinc-950/60 border border-zinc-900 px-4 py-2 rounded-2xl">
            <div className="w-2 h-2 rounded-full bg-red-500 animate-ping" />
            <span className="text-[11px] font-mono font-bold tracking-widest text-red-400">{currentTime || '--:--:--'}</span>
            <span className="text-[10px] text-zinc-500 font-bold border-r border-zinc-850 pr-2 mr-1">توقيت مكة</span>
          </div>
        </div>

        {/* CONTROLS AREA */}
        <div className="flex flex-col md:flex-row items-stretch md:items-center justify-between gap-4 bg-zinc-950/60 backdrop-blur-xl border border-zinc-90 w-full p-4 rounded-3xl">
          {/* Filters */}
          <div className="flex items-center gap-2 overflow-x-auto no-scrollbar scroll-smooth">
            <button
              onClick={() => setActiveFilter('all')}
              className={`px-4 py-2.5 rounded-2xl text-[11px] font-bold whitespace-nowrap transition-all cursor-pointer ${
                activeFilter === 'all' 
                  ? 'bg-red-600 text-white shadow-lg shadow-red-600/15 font-black' 
                  : 'bg-zinc-900 hover:bg-zinc-800 text-zinc-400 hover:text-white'
              }`}
            >
              المباريات المتاحة ({matches.length})
            </button>
            <button
              onClick={() => setActiveFilter('live')}
              className={`px-4 py-2.5 rounded-2xl text-[11px] font-bold whitespace-nowrap transition-all flex items-center gap-1.5 cursor-pointer ${
                activeFilter === 'live' 
                  ? 'bg-red-600 text-white shadow-lg shadow-red-600/15 font-black' 
                  : 'bg-zinc-900 hover:bg-zinc-800 text-zinc-400 hover:text-white'
              }`}
            >
              <span className="w-2.5 h-2.5 rounded-full bg-red-500 border-2 border-white animate-ping shrink-0" />
          ...
          <div className="flex items-center gap-2">
            <div className="relative flex-1 md:w-64">
              <input
                type="text"
                placeholder="ابحث عن خصم، بطولة، معلق..."
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                className="w-full bg-[#0d0d0f] border border-zinc-900 rounded-2xl px-4 py-2 text-[11px] text-right text-white focus:outline-none focus:border-red-600 placeholder-zinc-500 transition-colors"
              />
            </div>
            <button 
                ...
                >
                  ...
                  <div className="relative z-10 border-t border-zinc-900/60 pt-4 mt-6 flex items-center justify-between gap-3 flex-wrap">
                    ...
                    >
                      ...
                      <span>شاهد المباراة والدردشة</span>
                    </button>
                  </div>
                </motion.div>
              ))}

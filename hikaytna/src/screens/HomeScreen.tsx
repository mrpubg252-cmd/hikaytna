...
import { initializeEpisodeTracking, hasNewEpisode, getEpisodeUpdatedAt, markSeriesAsRead, getLastNewDetectedAt } from '../lib/episodeHistory';
...
export default function HomeScreen() {
  ...
  async function loadData() {
    try {
      ...
      initializeEpisodeTracking(data);
      setAllSeries(data);
  ...
  function handleCategoryChange(category: string) {
    ...
    const sorted = [...filtered].sort((a, b) => {
      // 1. Dynamic Priority: If Category is NOT search results ('نتائج البحث'), 
      // prioritize series with "New Episode" (hasNewEpisode) to the absolute top of the category,
      // and sort those with New Episodes by the newest updatedAt (descending).
      if (category !== 'نتائج البحث') {
        const hasNewA = hasNewEpisode(a);
        const hasNewB = hasNewEpisode(b);
        if (hasNewA && !hasNewB) return -1;
        if (!hasNewA && hasNewB) return 1;
        if (hasNewA && hasNewB) {
          const timeA = getEpisodeUpdatedAt(a);
          const timeB = getEpisodeUpdatedAt(b);
          return timeB - timeA; // newer timestamp at the top!
        }

        // Active persistent check: sort by the time a new episode was detected (even if read!)
        const detA = getLastNewDetectedAt(a);
        const detB = getLastNewDetectedAt(b);
        if (detA !== detB) {
          return detB - detA; // newer detected episode always higher in category!
        }
      }

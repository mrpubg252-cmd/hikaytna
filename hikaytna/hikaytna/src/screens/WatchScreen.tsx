    ...
    const handleResize = () => {
      if (window.visualViewport) {
        ...
        if (isChatOpen || isMaximized) {
          window.scrollTo(0, 0); // Prevent body from scrolling away from fixed canvas
        }
      }
    };
      ...
      <div className="max-w-7xl mx-auto flex flex-col items-center">
        ...
        <div className="w-full px-4 sm:px-8 flex flex-col lg:flex-row gap-8 sm:gap-12">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 sm:gap-6">
              ...
              <div className="w-full lg:w-[420px] shrink-0 order-last lg:order-none">
                <button
                  ...
                  className="w-full flex items-center justify-between gap-4 bg-gradient-to-br from-zinc-900/40 to-black/60 hover:from-zinc-900 hover:to-zinc-950 border border-white/5 hover:border-primary/30 p-4 rounded-3xl cursor-pointer group transition-all text-right shadow-xl"
                >
                  <div className="flex items-center gap-3">
                    <div className="w-12 h-12 bg-primary/10 rounded-2xl flex items-center justify-center border border-primary/20 text-primary group-hover:scale-110 group-hover:bg-primary/20 transition-all shrink-0">
                      <MessageSquare className="w-6 h-6" />
                    </div>
                    <div>
                      <h3 className="text-sm font-black text-white flex items-center gap-1">
                        مجلس المتابعين والدردشة
                        <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse" />
                      </h3>
                      <p className="text-[11px] text-zinc-400 font-bold mt-1">تفاعل ودردش وعلق على اللقطات مع المشاهدين الآخرين</p>
                    </div>
                  </div>
                  <ChevronRight className="w-4 h-4 text-zinc-500 group-hover:text-primary transition-all rotate-180 shrink-0" />
                </button>
              </div>
            </div>
          ...
          <aside className="w-full lg:w-96 flex flex-col gap-6">
            {/* Always-open, compact custom Series Chat block */}
            <div 
              ref={chatRef}
              className="bg-gradient-to-br from-[#0c0c12] to-[#050508] border border-white/5 rounded-3xl shadow-2xl overflow-hidden flex flex-col h-[480px]"
            >

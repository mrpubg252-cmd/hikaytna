...
import { Send, Users, Sparkles, Smile, Clock, User2, RefreshCw, Mic, Square, Volume2, Wand2, X, MessageSquare, Share2, Camera, Reply, ArrowLeft, LogIn, Shield } from 'lucide-react';
...
export default function SeriesChat({ seriesId, seriesTitle, onClose, currentPlaybackTime, onSeekTo, isGlobal, seriesImage }: SeriesChatProps) {
  ...
  // Real-time Presence, Typing indicators, and Guest Authentication modal states
  ...
  const userName = user ? (user.fullName || user.username || user.firstName) : 'مشاهد';
  const userAvatar = user?.imageUrl || 'boy1';
  ...
  const setTypingStateInDb = (isTyping: boolean) => {
    ...
    const presenceId = user ? user.id : (localStorage.getItem('guest_chat_pid') || 'guest_temp');
  ...
  useEffect(() => {
    ...
    let presenceId = user ? user.id : localStorage.getItem('guest_chat_pid');
    if (!presenceId) {
    ...
    const unsubscribeTyping = onValue(typingListRef, (snapshot) => {
      ...
      const now = Date.now();
      const currentPresenceId = user ? user.id : localStorage.getItem('guest_chat_pid');
  ...
  }, [seriesId, isDbReady, user, userName]);
  ...
  useEffect(() => {
    // Stale logic removed since we use useAuth now
  }, []);
  ...
  return (
    <div className="w-full bg-[#0d0d12] sm:rounded-3xl rounded-none border-0 sm:border border-white/5 overflow-hidden flex flex-col h-full shadow-[0_20px_50px_rgba(0,0,0,0.8)] relative font-sans">
      {/* Header */}
        ...
        <div className="flex items-center gap-3">
          <div className="text-right">
            <h3 className="text-xs sm:text-sm font-black text-white flex items-center gap-1.5 justify-end">
              مجلس المتابعين
              <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-[9px] text-emerald-400 font-extrabold animate-pulse">
      ...
      <div className="flex-1 relative overflow-hidden flex flex-col bg-zinc-950">
        ...
        <AnimatePresence>
          {isAuthModalOpen && (
            <motion.div 
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              className="absolute inset-0 bg-black/95 backdrop-blur-md z-30 overflow-y-auto p-4 flex flex-col items-center justify-start sm:justify-center text-right"
            >
              <div className="w-full flex justify-end mb-2 max-w-sm">
                <button 
                  onClick={() => setIsAuthModalOpen(false)}
                  className="px-4 py-1.5 bg-zinc-900 border border-white/10 hover:bg-zinc-800 text-white text-xs font-bold rounded-full cursor-pointer flex items-center gap-1.5"
                >
                  <ArrowLeft className="w-3.5 h-3.5" />
                  <span>تراجع للدردشة</span>
                </button>
              </div>
              <div className="w-full max-w-sm py-8 sm:py-4 my-auto">
                <AuthContainer />
              </div>
            </motion.div>
          )}
        ...
        <div className="bg-[#121218] border-t border-white/5 pb-[calc(1rem+env(safe-area-inset-bottom))] lg:pb-0">
          ...
          {isRegistered && (
            <div className="px-4 py-1.5 flex items-center gap-2 overflow-x-auto no-scrollbar select-none">
              {/* @حكيم Sparkle button */}
              <button 
                type="button"
                onClick={() => setInputText(prev => prev.includes('@حكيم') ? prev : `@حكيم ${prev}`)}
                className="flex items-center gap-1 px-2.5 py-1 rounded-full text-[10px] font-black transition-all shrink-0 border bg-primary/10 text-primary border-primary/25 hover:bg-primary/20 hover:scale-105 active:scale-95"
              >
                <Sparkles className="w-3 h-3 fill-current animate-pulse" /> 
                اسأل حكيم بالذكاء الاصطناعي
              </button>
              ...
              {!isGlobal && (
                <button 
                  ...
                  onClick={shareScene} 
                  className={cn(
                    "flex items-center gap-1 px-2.5 py-1 rounded-full text-[10px] font-black transition-all shrink-0 border",
                    pendingScene 
              ...
              {['🔥 حماس', '😍 أسطوري', '😭 حزن'].map(e => (
                <button type="button" key={e} onClick={() => { setInputText(e); setTimeout(() => handleSendMessage(), 50); }} className="px-2.5 py-1 bg-zinc-900 rounded-full text-[10px] text-zinc-400 border border-white/5 hover:border-zinc-700 transition-all shrink-0 hover:text-white">{e}</button>
              ))}
            </div>
          )}
          ...
          {!isRegistered ? (
            <div className="p-3 flex gap-2 items-center">
              <input 
                ...
                dir="rtl" 
                disabled 
                placeholder="سجل دخولك لتتمكن من المشاركة في الدردشة والتعليق..." 
                className="flex-1 bg-white/[0.02] border border-white/5 rounded-full py-2.5 px-5 text-xs text-zinc-500 outline-none placeholder-zinc-600 font-semibold cursor-not-allowed select-none" 
              />
              <button 
                type="button" 
                onClick={() => setIsAuthModalOpen(true)} 
                className="h-10 px-4 sm:px-5 bg-primary text-black hover:bg-primary/95 transition-all font-black text-xs rounded-full flex items-center justify-center gap-1.5 cursor-pointer shadow-lg active:scale-95 shrink-0"
              >
            ...
            <form onSubmit={handleSendMessage} className="p-3 flex gap-2 items-center">
              {isRecording ? (
                ...
                <>
                  ...
                  <input type="text" dir="rtl" value={inputText} onChange={(e) => setInputText(e.target.value)} placeholder={pendingScene ? "اكتب تعليقك على اللقطة..." : editingMsg ? "اكتب التعديل..." : "اكتب تعليقك هنا..."} className="flex-1 bg-white/5 border border-white/5 rounded-full py-2.5 px-5 text-xs outline-none text-white focus:ring-1 focus:ring-primary/40 placeholder-zinc-600 font-semibold" />
